...
6. **Native addons self-heal on Node changes (joelazar trick).** `45-rebuild-native-modules.sh.tmpl`
   ...
   (the ABI becomes a _content key_ — the change triggers re-run; the `2>/dev/null` guard variant
...
| Script                      | Type                       | Purpose                                                                 |
| --------------------------- | -------------------------- | ----------------------------------------------------------------------- |
| `00-bootstrap`              | `run_once_before_`         | package manager present **or installed** (brew auto-installed), sudo -v |
...
are **by design** — `~/.config` _is_ today's repo worktree (its untracked files are hidden
